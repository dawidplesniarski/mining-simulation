...
const MainPage = () => {
  ...
  const updateMiners = () => {
    ...
    setBlockElements(belements);

  };
  const addBlock = (belements) => {
    const newId =
      Number.parseInt(
        belements.filter((e) => !e.target)[
          belements.filter((e) => !e.target).length - 1
        ].id,
        10
      ) + 1;
    ...
    const block = {
      ...
      type: "input",
      data: {
        label: <>hash: {generateHash()} {newId.toString()}</>,
      },
    ...
    belements = [...belements, block,edge1]
    ...
    <MainView>
      <RowWrapper>
        ...
        <MinersGraphSceneWrapper>
          <input
            placeholder={"tutaj klikaj enter i spacja xD"}
            style={{ opacity: 1 }}
            type={"text"}
            onKeyDown={(e) => (e.key === "Enter" ? addMiner() : updateMiners())}
          />
